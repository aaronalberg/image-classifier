...
int classifyImage(Image &image, vector<double> &class_proportions,
  ...
  for (int num = 0; num < kNumClasses; num++) {
    comparable_probs[num] =  comparable_probs[num] + class_proportions[num];
    for (int i = 0; i < kImageSize; i++) {
      for (int j = 0; j < kImageSize; j++) {
        if (image.pixels[i][j] == 1) {
          comparable_probs[num] = comparable_probs[num] + log(pixel_proportions[num][i][j]);
        } else {
  ...
  for (int i = 0; i < kNumClasses; i++) {
    if (comparable_probs[i] > highest_value) {
      highest_index = i;
      highest_value = comparable_probs[i];
    }
  }
